...
/*  
    ...
    echo "Bimestre= ".$bimestre2; echo "<br />";
    echo "Turma= ".$turmaatual; echo "<br />";
...
$sql3 = "SELECT * FROM notas WHERE Matricula = '$matricula' && Ano = '$ano' && Materia = '$materia2[$n]'";
	  ...
	  if ($bimestre2 == 1){
	   ...
	   $query = mysqli_query($conexao, $sql = "UPDATE notas SET Bim1='$notas1[$n]',Faltas1='$faltas1[$n]' WHERE Matricula = '$matricula' && Materia = '$materia2[$n]' && Ano = '$ano'") or die(mysql_error());
	   }
	  if ($bimestre2 == 2){
	   $notas2 = $_POST['Notas'];
	   ...
	   if($media[$n] < 6)
		  ...
		  $rec = '-'; 
	   $query = mysqli_query($conexao, $sql = "UPDATE notas SET Bim2='$notas2[$n]',Faltas2='$faltas2[$n]', Media='$media[$n]', Recuperacao = '$rec' WHERE Matricula = '$matricula' && Materia = '$materia2[$n]' && Ano = '$ano'") or die(mysql_error());
	   }
	  if ($bimestre2 == 3){
	   $notas3 = $_POST['Notas'];
	   ...
	   if($media[$n] < 6)
		  ...
		  $rec = '-'; 
	   $query = mysqli_query($conexao, $sql = "UPDATE notas SET Bim3='$notas3[$n]',Faltas3='$faltas3[$n]', Media='$media[$n]', Recuperacao = '$rec' WHERE Matricula = '$matricula' && Materia = '$materia2[$n]' && Ano = '$ano'") or die(mysql_error());
	   }
	  if ($bimestre2 == 4){
	   $notas4 = $_POST['Notas'];
	   $faltas4 = $_POST['Faltas'];
	   $media = ($bim1 + $bim2 + $bim4 + $notas4[$conta])/4;
	   $media = number_format(round($media * 2 )/ 2,1);
	    if($media[$n] < 6)
	      $rec = 'Sim';
		 else
		  $rec = '-'; 
	    $query = mysqli_query($conexao, $sql = "UPDATE notas SET Bim4='$notas4[$n]',Faltas4='$faltas4[$n]', Media='$media[$n]', Recuperacao = '$rec' WHERE Matricula = '$matricula' && Materia = '$materia2[$n]' && Ano = '$ano'") or die(mysql_error()); 
	   }
	   if ($bimestre2 == 5){
	   $notas5 = $_POST['Notas'];
	   $media = $notas5;
...
if ($turmaatual == '1ano')
 ...
 if ($turmaatual == '1ano' && $retorno = 'r')
  ...
  else if ($turmaatual == '61ano' && $retorno <> 'r')
  	echo "<script type = 'text/javascript'> location.href = 'Notas6ano.php'</script>";	
