...
function checa(){
     if (document.cadastro.Matricula.value != "" && document.cadastro.Nome.value != "" && document.cadastro.Nascimento.value != "" && document.cadastro.Turma.value != "" && document.cadastro.Segmento.value != ""){
        ...
        alert("Os Campos com * não pode ficar vazios");
}
}
...
$nome2 = "Camilly Vitoria de Moura Santos";
$nasc = "23/06/2006";
$ano = 2013;
if(empty($nome2)){
	$nome2 = $_POST["Nome2"];
...
?>
 <cente>
        <form action="#" method="post" enctype="multipart/form-data" name="cadastro" onSubmit="return ValidaSemPreenchimento(this)">
	<fieldset id="forms">
		...
		<input type="text" name="Data" id="Data" class="textbox2"  tabindex="4" onKeyUp="Formatadata(this,event);"  size="14" maxlength="10" title="Informe a data da renovação da matrícula"/><br />
		<br />
        ...
        <select name="Transferencia"  class="textbox3" accesskey="s" tabindex="6" title="Informe a origem da inclusão">
         ...
         <label for="Turma">*Turma:</label><select name="Turma" id="Turma" class="textbox2" obrigatorio="1" tabindex="7" onChange="ajaxGet('processConsultaSeg.php?Turma='+this.value, document.getElementById('Segmento'), true)" >
           <option value="">Selecione</option>
        				<?php
			            //Busco os estados
