...
include("conexao.php");
	...
	$sql = "SELECT Matricula FROM histMatr WHERE Matricula = '$matricula' && Ano = '$ano2'";
...
while ($linha = mysqli_fetch_array($resultado))
{
 ...
 $sql = "INSERT INTO histMatr(Chamada,Matricula,Nome,Nascimento,Data,Ano,Turma,Transferencia,Segmento)	VALUES(0,'$matricula','$nome2',STR_TO_DATE('$nascimento','%d/%m/%Y'),STR_TO_DATE('$data1','%d/%m/%Y'),'$ano','$turma','$transferencia','$segmento')";
 
 $result = mysqli_query($conexao, $sql) or die ("Histórico do Aluno não realizado.");
 
 $sql2 = "INSERT INTO pagamento(Matricula,Ano,Carne) VALUES('$matricula','$ano','$carne')";
...
$sql2 = "INSERT INTO histMatr(Matricula,Nome,Data,Ano,Turma,Transferencia,Segmento) VALUES('$matricula','$nome2',STR_TO_DATE('$data1','%d/%m/%Y'),'$ano','$turma','$transferencia',$segmento)";
 
 $result2 = mysqli_query($conexao, $sql2) or die ("Cadastro de histórico não realizado.");
